...
using .networks
config::MuZeroConfig #TODO

"""
Select the child with the highest UCB score.
"""
function select_child(config,node::Node, treeminmax::TreeMinMax)
...
"""
function ucb_score(config, parent::Node, child::Node, treeminmax::TreeMinMax)
    ...
    if child.visit_count > 0
        # Mean value Q
        value_score = normalize_tree_value(treeminmax, child.reward + (config.discount * (len(config.players) == 1 ? node_value(child) : -node_value(child))))
    else
        value_score = 0
    end
...
to the root.
"""
function backpropagate(config, search_path, value, to_play, treeminmax)
...
function run_mcts(config,
    ...
    for _ = 1:config.num_simulations
        ...
        value, reward, policy_logits, hidden_state = model.recurrent_inference() #TODO
