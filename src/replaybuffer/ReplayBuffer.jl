...
"""
function compute_target_value(config::Config, history::GameHistory, index::Int64)::Float64
...
function make_target(
    ...
    for current_index = state_index:state_index+config.num_unroll_steps
        ...
        elseif current_index == length(history.root_values)
            ...
            append!(
                target_policies,
                [
                    1 / length(history.child_visits[0]) for
                    _ = 1:length(history.child_visits[0])
                ],
            )
            append!(actions, history.action_history[current_index])
        else
...
function update_history!(config::Config, history::GameHistory, buffer::Dict, game_id::Int64)::nothing
    # The element could have been removed since its selection and training
    if keys_buffer[i] ≤ game_id #TODO
        if config.PER
...
"""
function sample_position(config::Config, history::GameHistory, force_uniform=false)::Tuple{Int64,Float64}
    position_prob = nothing
    if config.PER && !force_uniform
...
function sample_n_games(config::Config, buffer::Dict, n_games::Int64, force_uniform=false)::Vector{Tuple}
    if config.PER && !force_uniform
        game_id_list = []
        game_probs = Float32[]
...
function sample_game(config::Config, buffer::Dict, force_uniform=false)::Tuple
    game_prob = nothing
    if config.PER && !force_uniform
        game_probs = Float32[]
        for (game_id, history) in buffer
...
function get_batch(config::Config, buffer)::Tuple
    ...
    for (game_id, history, game_prob) in sample_n_games(config,buffer,config.batch_size)
        ...
        values, rewards, policies, actions = make_target(config,history, game_pos)
        ...
        append!(gradient_scale_batch,repeat(minimum(config.num_unroll_steps,length(history.action_history)-game_pos),length(actions)))
        config.PER ? append!(weight_batch,1/(total_samples*game_prob*pos_prob)) : nothing #TODO
    end
    weight_batch =  config.PER ? weight_batch ./ maximum(weight_batch) : nothing
    return index_batch, (
                observation_batch,
                action_batch,
                value_batch,
...
function save_game(config::Config, replay_buffer::ReplayBuffer,history::GameHistory,shared_storage=nothing)
    if config.PER
        if history.priorities ≠ nothing
            ...
            for (i,root_value) in enumerate(history.root_values)
                priority =  abs(root_value - compute_target_value(config, history,i)) ^ config.PER_alpha
        ...
        if config.replay_buffer_size < length(buffer)
        ...
        if shared_storage
            set_info!("num_played_games",replay_buffer.num_played_games)
            set_info!("num_played_steps",replay_buffer.num_played_steps)
        end
    end
end